...
float u1 = 0.0;
float u2 = 0.0;
float p1_current = 0.0;
float p1_old = 0.0;
float p2_current = 0.0;
float p2_old = 0.0;
float v1 = 0.0;
float v2 = 0.0;
float pos = 0.0;
...
void control(void) {
    ...
    count++;
    p1_current = Enc1_rad/192.0;
    p2_current = Enc2_rad/192.0;
    ...
    switch (switchstate){
    case 0: //all switches backwards
        u1 = Enc3_rad/10;
        u2 = Enc3_rad/10;
        PWM_out(EPWM1,u1);
        PWM_out(EPWM2,-u2);
        if ((count%250)==0){
            LCDPrintfLine(1,"Open Loop");
            LCDPrintfLine(2,"V:%.1f L:%.1f R:%.1f",u1,v1,v2);
        }
        break;
    case 15: //all switches forwards
        PWM_out(EPWM1,0);
        PWM_out(EPWM2,0);
        if ((count%250)==0){
    ...
    case 13: // Closed-Loop
        ...
        i1 = i1_old + ( (err1+err1_old)/2 )*0.001;
        i2 = i2_old + ( (err2+err2_old)/2 )*0.001;
        ...
        scale = 0.6;
        if (v1>0.0) {
            u1 = u1 + (2.18*v1 + 2.05)*scale;
        }
        else {
            u1 = u1 + (2.2*v1 - 2.1)*scale;
        }
        if (v2>0.0) {
            u2 = u2 + (2.18*v2 + 2.05)*scale;
        }
        else {
            u2 = u2 + (2.2*v2 - 2.1)*scale;
        }
        ...
        if ((count%250)==0){
            LCDPrintfLine(1,"CLosed Loop");
            LCDPrintfLine(2,"V:%.1f L:%.1f R:%.1f",vref,v1,v2);
        }

        break;
    case 11: // Closed-Loop with Coupled Control Loops
        // Friction Scaling
        vref = Enc3_rad/10;
        turn = 0;

        err1 = vref-v1 + e_steer*Kp_turn;
        err2 = vref-v2 - e_steer*Kp_turn;
        ...
        if ((count%250)==0){
            LCDPrintfLine(1,"Coupled CLosed Loop");
            LCDPrintfLine(2,"V:%.1f L:%.1f R:%.1f",vref,v1,v2);
        }
        break;
    case 7: // Driving around
        // Friction Scaling
        vref = 1.0;
        turn = -Enc3_rad/10;
        ...
        if ((count%250)==0){
            LCDPrintfLine(1,"Coupled PI Turn Loop");
            LCDPrintfLine(2,"T:%.1f L:%.1f R:%.1f",turn,v1,v2);
        }
        break;
    }
