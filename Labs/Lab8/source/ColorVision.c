...
void init_ColorVision(void) {
    ...
    case 0: //green
        ...
        specs_s = (160+255)/2;
        specs_srad = (255-160)/2;
        specs_v = (110+255)/2;
        specs_vrad = (255-110)/2;
        break;
    case 1: //blue
        specs_h = (140+170)/2;
        specs_hrad = (170-140)/2;
        if((specs_h-specs_hrad)<0) // wrap 0->360
        {
            specs_h2=specs_h+256;
        }
    ...
    if (ptrImage != NULL) {
        ...
        for (r=0;r<IMAGE_ROWS;r++) {
            for(c=0;c<IMAGE_COLUMNS;c++) {
                ...
                //if (ColorToggle == 0) { //go after blue
                //spec_h = (140+170)/2;
        ...
        if (new_coordata == 0) {
            if (final_object_stats[largest_object].num_pixels_in_object > 3) {
                noimagefound = 0;
                ...
                new_coordata = 1;
                if (colorstate == 0){
                    green_object_x = object_x;
                    green_object_y = object_y;
                    green_numpels = numpels;
                    colorstate = 1;
                }else {
