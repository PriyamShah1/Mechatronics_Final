...
void userProcessColorImageFunc_laser(bgr *ptrImage) {
	...
	if (ptrImage != NULL) {
		...
		for (r = 0; r<(144-30);r++) {
		    for (c = 0; c<176; c++) {
		        myred = ptrImage[r*IMAGE_COLUMNS+c].red;
		        myblue = ptrImage[r*IMAGE_COLUMNS+c].blue;
		        mygreen = ptrImage[r*IMAGE_COLUMNS+c].green;

		        if ((myblue > 200) && (myred>200) && (mygreen>200)){ //white pix
		            rowsum+= r;
		            colsum+= c;
		            pixsum++;
		        }
		...
		if (pixsum!=0){
	        centerR = rowsum/pixsum;
	        centerC = colsum/pixsum;
		}else{
		    centerC= 88;
		    centerR= 72;
		}
		...
		//5px crosshair
		if ((centerR!=0) && (centerR!=144) && (centerC!=0) && (centerC!=176)) {
	        for (r=(centerR-1);r<=(centerR+1); r++) {
	            ptrImage[r*IMAGE_COLUMNS+centerC].green = 255;
	            ptrImage[r*IMAGE_COLUMNS+centerC].blue = 0;
	            ptrImage[r*IMAGE_COLUMNS+centerC].red = 0;
	        }
            for (c=(centerC-1);c<=(centerC+1); c++) {
                ptrImage[centerR*IMAGE_COLUMNS+c].green = 255;
                ptrImage[centerR*IMAGE_COLUMNS+c].blue = 0;
                ptrImage[centerR*IMAGE_COLUMNS+c].red = 0;
            }
		}
