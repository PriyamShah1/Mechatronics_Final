...
float value_enc1 = 0;
float value_enc2 = 0;
float b[8]={    1.2296498873159978e-06, 8.6075492112119845e-06, 2.5822647633635953e-05, 4.3037746056059922e-05, 4.3037746056059922e-05, 2.5822647633635953e-05, 8.6075492112119845e-06, 1.2296498873159978e-06};
float a[8]={    1.0000000000000000e+00, -5.5889821399350339e+00,    1.3504744699803794e+01, -1.8270852517801739e+01,    1.4936521932451939e+01, -7.3738011768792617e+00,    2.0343997965247613e+00, -2.4187319897888210e-01};
...
void Adc_isr(void) {
    // Read 10 ADC readings from conversion results
    ...
    // Start read process of LS7366 optical encoder chips
    start_SPI();
...
void control(void) {
    ...
    // writeDAC7564((raw_adc_B1/4095.0)*3.0,(raw_adc_B2/4095.0)*3.0);
    /*
    float xsum = 0;
    int i;
    for (i = 7 ; i>0 ; i = i-1) {
    ...
    xvec[0] = raw_adc_B1;
    xsum += xvec[0];
    writeDAC7564(((xsum/8)/4095.0)*3.0,(raw_adc_B2/4095.0)*3.0);
    */

    x[0] = (raw_adc_B1/4095.0)*3.0; //adc voltage is the current sample

    // calculate filter value
    y[0]=0;
    int i = 7;
    for ( i=7; i>=0; i=i-1) {
        y[0] += b[i]*x[i]; // + b[1]*x[1] + b[2]*x[2] + b[3]*x[3] + b[4]*x[4]+ b[5]*x[5]+ b[6]*x[6]+ b[7]*x[7]
        if (i != 0 ) {
            y[0] -=  a[i]*y[i]; //- a[1]*y[1] - a[2]*y[2] - a[3]*y[3]- a[4]*y[4]- a[5]*y[5]- a[6]*y[6]- a[7]*y[7]
        }
    }
    ...
    F28335_Extra1 = (raw_adc_B1/4095.0)*3.0;
    F28335_Extra2 = (raw_adc_B2/4095.0)*3.0;
