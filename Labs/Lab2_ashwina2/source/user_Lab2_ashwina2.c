...
void sendtext(void) {
    clk_cnt++;
    if (0 == (clk_cnt % 250)) {
        //LCDPrintfLine(1,"Sem not called yet");
        LCDPrintfLine(2,"Time (ms): %d", clk_cnt);
    }
}
...
void ComWithLinux(void) {
	...
	while(1) {
	    ...
	    if (1 == GET_LVDATA_FROM_LINUX) {
		    // post semaphore
		    Semaphore_post(sem_taskcnt);

		    // read shared memory
		    Cache_inv((void *)ptrshrdmem,sizeof(sharedmemstruct), Cache_Type_ALL, EDMA3_CACHE_WAIT);

		    for (i = 0; i < ptrshrdmem->DSPRec_size; i++) {
		        shrdmemin[i] = ptrshrdmem->DSPRec_buf[i];
		    }

		    CLR_LVDATA_FROM_LINUX; // Set Flag to 0

		    // Edit string to send back to Linux/LabVIEW
		    size = sprintf(shrdmemout,"%s, Time (ms): %d", shrdmemin, clk_cnt);

		    if (1 == GET_LVDATA_TO_LINUX) {
		        for (i = 0; i < size; i++) {
		            ptrshrdmem->DSPSend_buf[i] = shrdmemout[i];
		        }
		        ptrshrdmem->DSPSend_size = size;
...
void SemTask(void) {
    while(1) {
        Semaphore_pend(sem_taskcnt,BIOS_WAIT_FOREVER);
        sem_cnt++;
        LCDPrintfLine(1,"Sem Calls: %d", sem_cnt);
    }
}
