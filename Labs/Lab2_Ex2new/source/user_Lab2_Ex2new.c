...
void LCD_screen(void){
    ...
    if (0==(timer_lcd%250)){
        ...
        //LCDPrintfLine(2,"Time passed: %d",timer_lcd);
    }
}
...
void Sema(void){
    while(1){
        Semaphore_pend(semtest,BIOS_WAIT_FOREVER);
        semcount++;
        LCDPrintfLine(1,"Robot ready to roll!");
        LCDPrintfLine(2,"Interrupt Count: %d",semcount);
    }
}
...
void ComWithLinux(void) {
    ...
    while(1) {
        ...
        if (GET_LVDATA_FROM_LINUX == 1){
            ...
            CLR_LVDATA_FROM_LINUX; // linux can now write into memory
            SET_LVDATA_TO_LINUX; // linux can now read memory

            MySize = sprintf(TXstring,"%s,Count = %d",MyString,timer_lcd);

            if (GET_LVDATA_TO_LINUX ==1){
                ptrshrdmem->DSPSend_size = MySize;
                ...
                CLR_LVDATA_TO_LINUX; // DSP can now read memory
            }
