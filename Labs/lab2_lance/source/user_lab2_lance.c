...
void LCD_clock(void){
    ...
    if (0==(lcd_time%250)){
        ...
        //LCDPrintfLine(2,"Time: %d",lcd_time);
    }
}
...
void semaphore(void){
    while(1){
        ...
        wake_count++;
        LCDPrintfLine(1,"Part3");
        LCDPrintfLine(2,"Wake Count: %d",wake_count);
    }
}
...
void ComWithLinux(void) {
	...
	while(1) {
	    ...
	    if (GET_LVDATA_FROM_LINUX == 1) {
	        ...
	        data_size = sprintf(TXstring,"%s,Count = %d",mystring,lcd_time);
s
	        if (GET_LVDATA_TO_LINUX == 1){
	            ptrshrdmem->DSPSend_size = data_size;
