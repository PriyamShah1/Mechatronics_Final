...
#warn GP6.15 is also connected to CAMERA RESET This is a Bug in my board design Need to change Camera Reset to different IO.
    ...
    robotdest[6].x = 0;     robotdest[6].y = 0;
    robotdest[7].x = 3;    robotdest[7].y = 7;
    //    //middle of bottom
...
void RobotControl(void) {
    ...
    if (settlegyro < SETTLETIME) {
        ...
        case 8:
            if ((timecount%250)==0){
                LCDPrintfLine(1,"%d %.1f %.1f",astarRunning,vref,turn);
                LCDPrintfLine(2,"%d",(int)(switchstate));
            }
            break;
        case 9:
        ...
        //            break;
        //        case 2:
        if( xy_control(&vref, &turn, 1.0, ROBOTps.x, ROBOTps.y, pathCol[pathPos], pathRow[pathPos], ROBOTps.theta, 0.25, 0.5))
        {
            pathPos++;
            if (pathPos == pathLen) {
                statePos = (statePos+1)%robotdestSize;
    ...
    if(ROBOTps.y > obs[index].y){
            for(i = 0; i < 30; ++i){
                if((boxes[i].edge[0]->x == obs[index].x) && (boxes[i].edge[0]->y == obs[index].y)){
                    ...
                    if((!(boxes[i].no_left_wall)) && (!(boxes[i].no_right_wall)) && (!(boxes[i].no_bottom_wall))){
                        for(j = 0; j < 4; j ++){
                    ...
                    else if((boxes[i].no_bottom_wall == 1) && (boxes[i].no_left_wall == 1)){
                    ...
                    else if((boxes[i].no_bottom_wall == 1) && (boxes[i].no_right_wall == 1)){
                        map[boxes[i].edge[0]->idx1] = 'x';
                        map[boxes[i].edge[0]->idx2] = 'x';
